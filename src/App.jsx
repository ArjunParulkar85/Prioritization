    ...
    <div style={{ minHeight:"100vh", background:theme.background, color:theme.color, fontFamily:"system-ui, Arial", padding:24 }}>
      <div style={{ maxWidth:1200, margin:"0 auto" }}>
                ...
                <tr key={r.id} style={{ borderTop:`1px solid ${theme.border}` }}>
                  ...
                  {["impact","ttv","feasibility","data","risk","align","buyin","cost"].map(k=>(
                    <td key={k} style={{ padding:6 }}>
                      <input type="number" min="0" max="5" value={r[k]??0} onChange={(e)=>updateRow(r.id,{ [k]:Number(e.target.value) })} style={{ width:60 }} />
                    </td>
                  ))}
        ...
        <div style={{ marginTop:12 }}>
          <button onClick={addRow} style={{ padding:"8px 12px", background:BRAND, color:"#fff", border:"none", borderRadius:8, cursor:"pointer" }}>
            Add Row
          </button>
        </div>
      </div>
