...
function computeScore(row, W) {
  ...
  const E = Math.max(1, Number(row.effort || 1));
...
// Temporary readable UID (when Trello idShort not yet known)
...
// CXO meta encoding in description
...
export default function App() {
  ...
  // auto-load from GCP on mount (silent)
  useEffect(() => { loadFromCloud(true); }, []);

  // auto-save every 5 minutes (silent)
  useEffect(() => {
  ...
  const scored = useMemo(
    () => rows.map(r => ({ ...r, score: computeScore(r, weights).score })),
    [rows, weights]
  );
  ...
  async function saveToCloud(silent = false) {
    try {
      ...
      if (!silent) setStatus(`❌ Save failed: ${e.message || e}`);
    }
  }
      ...
      try {
        ...
        if (card?.id && (card.idShort || card.shortLink)) {
          ...
          const newMeta = buildMetaLine({
            impact: r.impact, reach: r.reach, effort: r.effort,
            urgency: r.urgency, align: r.align, uid: trueUid
          });
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.background, color: theme.text, fontFamily: "Inter, ui-sans-serif, system-ui, Arial", padding: "12px 12px" }}>
              ...
              <thead>
                <tr style={{ textAlign: "left", color: theme.muted }}>
                  ...
                  <th className="cell col-num">
                    <HeaderWithHelp label="Impact" tip="How much this moves key business goals AND solves a real user problem. 1=min, 5=max." />
                  </th>
                  <th className="cell col-num">
                    <HeaderWithHelp label="Reach" tip="How many users will be affected. 1=few, 5=all/most." />
                  </th>
                  <th className="cell col-num">
                    <HeaderWithHelp label="Effort" tip="Total team work. XS=1, S=2, M=3, L=5, XL=8. Higher effort lowers score." />
                  </th>
                  <th className="cell col-num">
                    <HeaderWithHelp label="Urgency" tip="Criticality right now to unblock work or seize timing. Low=1…Critical=4." />
                  </th>
                  <th className="cell col-num">
                    <HeaderWithHelp label="Alignment" tip="Strategic fit with current goals. 1=none, 5=perfect." />
                  </th>
      ...
      <div className={`drawer ${showWeights ? "open" : ""}`}>
        ...
        <div style={{ fontSize: 12, color: theme.muted }}>Tip: all 1’s give a simple model. Raise a slider to emphasize that factor.</div>
      </div>
